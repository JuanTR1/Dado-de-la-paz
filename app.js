...
function renderGrid(){
 ...
 grid.innerHTML=faces.map((face,index)=>`<button class="value-card ${current===index?'active':''}" style="--accent:${face.color}" type="button" data-index="${index}"><span class="value-image"><img src="${face.image}" alt="${face.text[lang][0]}" loading="lazy"></span><span class="value-copy"><span class="value-index">0${index+1}</span><h3>${face.text[lang][0]}</h3><p>${face.text[lang][1]}</p></span></button>`).join('');
 grid.querySelectorAll('.value-card').forEach(card=>card.addEventListener('click',()=>selectFace(Number(card.dataset.index),true)));
}
