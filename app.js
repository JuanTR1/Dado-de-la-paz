const faces=[
 ...
 {image:'assets/soy-el-primero-en-amar.png',color:'#5595c8',rotation:'rotateX(73deg) rotateY(0deg)',text:{es:['Soy el primero en amar','Da el primer paso sin esperar a que la otra persona lo haga.'],en:['I am the first to love','Take the first step without waiting for someone else.'],it:['Sono il primo ad amare','Fai il primo passo senza aspettare l’altra persona.'],fr:['Je suis le premier à aimer','Fais le premier pas sans attendre l’autre personne.'],ca:['Soc el primer a estimar','Fes el primer pas sense esperar l’altra persona.']}},
...
function renderGrid(){
 ...
 grid.innerHTML=faces.map((face,index)=>`<button class="value-card ${current===index?'active':''} ${flippedCards.has(index)?'flipped':''}" style="--accent:${face.color}" type="button" data-index="${index}" aria-label="${face.text[lang][0]}" aria-pressed="${flippedCards.has(index)}"><span class="value-card-inner"><span class="value-card-face value-card-front"><span class="value-image"><img src="${face.image}" alt="${face.text[lang][0]}" loading="lazy"></span><span class="value-copy"><span class="value-index">0${index+1}</span><h3>${face.text[lang][0]}</h3><p>${face.text[lang][1]}</p></span><span class="flip-cue"><span>Ver mensaje</span><span class="flip-icon" aria-hidden="true">↻</span></span></span><span class="value-card-face value-card-back"><span><span class="value-index">${ui[lang].today}</span><h3>${face.text[lang][0]}</h3><p>${face.text[lang][1]}</p></span><span class="flip-cue"><span>Volver</span><span class="flip-icon" aria-hidden="true">↺</span></span></span></span></button>`).join('');
...
function showMessage(index){
 ...
 setTimeout(()=>{resultImage.src=face.image;resultImage.alt=face.text[lang][0];resultTitle.textContent=face.text[lang][0];resultDescription.textContent=face.text[lang][1];messageCard.classList.remove('changing')},180);
}
...
function setLanguage(){
 ...
 document.querySelectorAll('[data-ui]').forEach(el=>el.textContent=ui[lang][el.dataset.ui]);showMessage(current);renderGrid();
}
